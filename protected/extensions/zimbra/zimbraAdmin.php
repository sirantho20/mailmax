...
class zimbraAdmin {
	...
	public function  __construct() {
                ...
                $this->server = Yii::app()->params['zimbraServer'];
		...
		curl_setopt($this->curlhandle, CURLOPT_SSL_VERIFYHOST, FALSE);
                ...
                $adminuser = Yii::app()->params['zimbraAdminLogin'];
                $adminpass = Yii::app()->params['zimbraAdminPassword'];
                $this->zimbra_login($adminuser, $adminpass);

	}

	private function set_zimbra_header() {
	...
	<GetAllAccountsRequest xmlns="urn:zimbraAdmin">
		...
		if (isset ($res['SOAP:ENVELOPE']['SOAP:BODY']['GETALLACCOUNTSRESPONSE']['ACCOUNT']))
                    {
			return $res['SOAP:ENVELOPE']['SOAP:BODY']['GETALLACCOUNTSRESPONSE']['ACCOUNT'];
	
		}
                elseif(isset ($res['SOAP:ENVELOPE']['SOAP:BODY']['GETALLACCOUNTSRESPONSE']))
                {
                    $this->zimbraerror = 'No records found or unknow error occured';
                    return false;
                }
		 ...
		 //	print_r($res['SOAP:ENVELOPE']['SOAP:BODY']['SOAP:FAULT']['SOAP:DETAIL']['ERROR']['CODE']['DATA']);
		 	...
		 	$this->zimbraerror = $res['SOAP:ENVELOPE']['SOAP:BODY']['SOAP:FAULT']['SOAP:DETAIL']['ERROR']['CODE']['DATA'];
                        return false;
		 }
	...
	<password>' . $password . '</password>';
		...
		if (isset ($res['SOAP:ENVELOPE']['SOAP:BODY']['CREATEACCOUNTRESPONSE']))
			...
			return $res['SOAP:ENVELOPE']['SOAP:BODY']['CREATEACCOUNTRESPONSE'];
                    ...
                    {
                        $this->zimbraerror = $res['SOAP:ENVELOPE']['SOAP:BODY']['SOAP:FAULT']['SOAP:REASON']['SOAP:TEXT']['DATA'];
                        return false;
                    }
	}
	...
	<account by="' . $type . '">' . $name . '</account>
		...
		if (isset ($res['SOAP:ENVELOPE']['SOAP:BODY']['GETACCOUNTRESPONSE']))
			return $res['SOAP:ENVELOPE']['SOAP:BODY']['GETACCOUNTRESPONSE']['ACCOUNT'];
		return false;
	}
 ...
 */
	...
	public function zimbra_is_domain_locked($domain) {
	...
	public function zimbra_is_account_locked($account) {
	...
	public function zimbra_lock_domain($domain) {
		return $this->zimbra_modify_domain($domain, array (
			'zimbraDomainStatus' => 'locked'
		));
	}

	public function zimbra_unlock_domain($domain) {
		return $this->zimbra_modify_domain($domain, array (
			'zimbraDomainStatus' => 'active'
		));
	}

	public function zimbra_lock_account($account) {
		return $this->zimbra_modify_account($account, array (
			'zimbraAccountStatus' => 'locked'
		));
	}

	public function zimbra_unlock_account($account) {
		return $this->zimbra_modify_account($account, array (
			'zimbraAccountStatus' => 'active'
		));
	}

	public function zimbra_get_id_from_domain($domain) {
		$out = $this->zimbra_get_domain($domain);
		if ($out == false)
			return false;
	...
	public function zimbra_get_id_from_server($server) {
	...
	public function zimbra_get_id_from_account($name) {
	...
	public function zimbra_get_id_from_cos($name) {
	...
	public function zimbra_get_aliases($email) {
		$out = array ();
