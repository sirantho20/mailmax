...
class ZimbraSOAP
{
    ...
    private function processReply($soapMessage)
    {
        ...
        if ($fault) {
            ...
            $code = $fault->Detail->children()->Error->Code;
            switch ($code) {
            case 'account.ACCOUNT_EXISTS':
                $this->err_msg = "account already exists";
                return false;
                break;
            case 'account.DISTRIBUTION_LIST_EXISTS':
                $this->err_msg = "distribution list already exists";
                return false;
                break;
            case 'service.PROXY_ERROR':
                $this->err_msg = "error while proxying request to target server";
                return false;
                break;
            default:
                $this->err_msg = "An unexpected error has occurred";
                return false;
        }
